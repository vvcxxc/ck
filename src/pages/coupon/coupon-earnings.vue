  ...
  <div class="coupon-earnings-box">
    ...
    <x-scroll :http="http"
      ...
      <group :title="`一共有 ${total} 条记录`"
             v-if="!!dataCouponEarnings.length">
        <div class="container">
          <x-list v-if="!!dataCouponEarnings.length"
                  v-for="(item, index) in dataCouponEarnings"
                  :key="index">
    ...
    <load-more v-if="!dataCouponEarnings.length"
               :show-loading="false"
               slot="inn"
               :tip="'暂无数据'"
...
export default {
  ...
  methods: {
    _coupnEarnings () {
      ...
      couponEarnings().then(({ code, data, last_page, total }) => {
        // console.log(data)
        if (code == 200) {
