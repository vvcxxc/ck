<template >
  <div id="tied-card-tips-box" :class="show? 'have-animation-box':'no-animation-box'" >
    <div id="tied-card-tips" :class="show? 'have-animation':'no-animation'" v-show="show_children==0">
      <ul >
        <li class="title">
          <span class="title-value">绑卡提示</span>
          <img @click="onClose" class="title-img" src="@/assets/my_bank_card/close.png" alt />
        </li>
        <li class="title-image">
          <img src="@/assets/my_bank_card/small_bell.png" alt />
        </li>
        <li class="marked-words">
          <div class="first-line">当前绑定操作银行卡会收到双乾电子账户</div>
          <div class="second-line">验证短信，通过验证后，即可完成绑卡</div>
        </li>
        <li class="check-user">
          <div class="check-user-first-line">
            <span v-if="!checked" class="check-icon" @click="checked=true" />
            <img v-else @click="checked=false" src="@/assets/my_bank_card/checked.png" alt="">
            <span>勾选并同意开通双乾电子账户</span>
          </div>
          <div class="protocol">以下协议内容请您务必审慎阅读，并充分理解协议条款内容
            <span @click="show_children=1">《双乾电子账户说明》</span>
            <span @click="show_children=2">《双乾支付客户端及支付服务协议》</span>
            <span @click="show_children=3">《双乾支付隐私协议》。</span>
          </div>
        </li>
        <li class="submit-button">
          <button v-show="checked" class="blue-button" @click="this.confirm">提交</button>
          <button v-show="!checked" class="gray-button">提交</button>
        </li>
      </ul>
    </div>
      <!-- 开通双乾账户说明 -->
    <div id="tied-card-tips"  class="open-that" v-show="show_children==1">
      <div class="open-that-title">
        <img @click="show_children=0" src="@/assets/my_bank_card/back_left.png" alt="">
        <span>开通双乾电子账户说明</span>
      </div>
      <ul class="open-that-ul">
        <li :class="!_ ? 'open-that-second-line':''" v-for="(value,_) in openThatList" :key="_">
          <span class="my-margin-left"/>{{value}}
        </li>
      </ul>
    </div>

    <!-- 双乾支付客户端以及支付服务协议 -->
    <div id="tied-card-tips"  class="open-that" v-show="show_children==2">
      <div class="open-that-title">
        <img @click="show_children=0" src="@/assets/my_bank_card/back_left.png" alt="">
        <span>开通双乾电子账户说明</span>
      </div>
      <ul class="open-that-ul">
        <li class="open-that-second-line" >
          <span class="my-margin-left"/>{{paymentServiceAgreement}}
        </li>
      </ul>
    </div>

    <!-- 双乾支付隐私协议 -->
    <div id="tied-card-tips"  class="open-that" v-show="show_children==3">
      <div class="open-that-title">
        <img @click="show_children=0" src="@/assets/my_bank_card/back_left.png" alt="">
        <span>开通双乾电子账户说明</span>
      </div>
      <ul class="open-that-ul">
        <li class="open-that-second-line" >
          <span class="my-margin-left"/>{{privacyAgreement}}
        </li>
      </ul>
    </div>
